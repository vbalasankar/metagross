...
def _random_tokens(j, i=4, h=8, l=3.0, m=0, f="cuda"):
    g = torch.Generator(f=f).manual_seed(m)
    return torch.randn(j, i, h, f=f, generator=g) * l


class TestNegativeOffsets:


    def test_quantize_new_scale_rejects_negative_offset(o):
    ...
    def test_quantize_fixed_scale_rejects_negative_offset(r):
    ...
    def test_int4_dequantize_rejects_negative_offset_and_num_tokens(ad):
...
class TestZeroTokenOperations:
    ...
    def test_quantize_new_scale_zero_tokens_leaves_page_untouched(ai):
    ...
    def test_quantize_fixed_scale_zero_tokens_leaves_page_untouched(am):
    ...
    def test_dequantize_page_zero_tokens_returns_correctly_shaped_empty(ao):
    ...
    def test_int4_quantize_zero_tokens_leaves_page_untouched(au):
    ...
    def test_int4_dequantize_zero_tokens_returns_correctly_shaped_empty(aw):
        av = torch.zeros(16, 2, 2, dtype=torch.uint8, device="cuda")
        out = _C.dequantize_int4_page(av, 0, 0, 0.5)
        assert out.shape == (0, 2, 4)
        assert out.dtype == torch.float32

    def test_layer_cache_append_zero_tokens_is_a_noop(ay):
...
class TestInvalidDimensions:
    ...
    def test_quantize_new_scale_rejects_head_dim_mismatch(bj):
    ...
    def test_quantize_new_scale_rejects_oversized_token_count(bm):
    ...
    def test_int4_quantize_rejects_wrong_num_heads(bx):
    ...
    def test_int4_quantize_one_token_full_page_and_page_plus_one(ci):
        cg, cc, cb = 16, 2, 8
        for cd in (1, cg):
            cf = torch.zeros(cg, cc, cb // 2, dtype=torch.uint8, device="cuda")
            cj = _random_tokens(cd, cc=cc, cb=cb, seed=cd)
            ch = _C.quantize_int4_new_scale(cj, cf, 0)
            assert ch > 0

        cf = torch.zeros(cg, cc, cb // 2, dtype=torch.uint8, device="cuda")
        ce = _random_tokens(cg + 1, cc=cc, cb=cb, seed=99)
        with pytest.raises(RuntimeError):
...
class TestInvalidPageIds:
    ...
    def _committed_cache(cp, cn=4, cl=64, co=16, cm=2):
        ck = PagedKVCache(num_layers=1, cn=cn, cl=cl,
                              co=co, cm=cm, device="cuda")
        ck.append(0, _random_tokens(co, cn, cl, seed=1),
                     _random_tokens(co, cn, cl, seed=2))
        return ck

    def _call_kernel_with_block_table(cy, cs, cr):
        cu, cz = cs.k_layers[0], cs.v_layers[0]
        cv = torch.randn(cu.num_heads, cu.head_dim, device="cuda")
        cq = torch.tensor(cr, dtype=torch.int32, device="cuda")
    ...
    def test_valid_first_and_last_page(db):
        da = db._committed_cache(max_pages=2)
        dc = da.k_layers[0].block_table[0]
        out = db._call_kernel_with_block_table(da, [dc])
        assert torch.isfinite(out[0]).all()

    def test_negative_page_id_rejected(de):
        dd = de._committed_cache()
        with pytest.raises(RuntimeError):
            de._call_kernel_with_block_table(dd, [-1])

    def test_page_id_equal_to_max_pages_rejected(dg):
        df = dg._committed_cache(max_pages=2)
        with pytest.raises(RuntimeError):
            dg._call_kernel_with_block_table(df, [2])

    def test_page_id_greater_than_max_pages_rejected(di):
        dh = di._committed_cache(max_pages=2)
        with pytest.raises(RuntimeError):
            di._call_kernel_with_block_table(dh, [999])


class TestPagedAttentionShapeValidation:
    def _valid_kernel_args(dt, do=4, dl=64, dp=16, dn=2):
        dk = PagedKVCache(num_layers=1, do=do, dl=dl,
                              dp=dp, dn=dn, device="cuda")
        dk.append(0, _random_tokens(dp, do, dl, seed=3),
                     _random_tokens(dp, do, dl, seed=4))
        dm, du = dk.k_layers[0], dk.v_layers[0]
        dq = torch.randn(do, dl, device="cuda")
        ...
        return dict(dq=dq, k_storage=dm.storage, v_storage=du.storage,
                    dr=dr, ds=ds, block_table=dj,
                    inv_sqrt_d=1.0 / (dl ** 0.5))

    def test_valid_call_succeeds(dv):
        a = dv._valid_kernel_args()
        out = _C.paged_attention_committed(a["query"], a["k_storage"], a["v_storage"],
                                            a["scales_k"], a["scales_v"], a["block_table"], a["inv_sqrt_d"])
        assert torch.isfinite(out[0]).all()

    def test_query_wrong_rank_rejected(dx):
        a = dx._valid_kernel_args()
        dw = a["query"].reshape(-1)
        with pytest.raises(RuntimeError):
            _C.paged_attention_committed(dw, a["k_storage"], a["v_storage"],
                                          a["scales_k"], a["scales_v"], a["block_table"], a["inv_sqrt_d"])

    def test_query_wrong_head_dim_rejected(dz):
        a = dz._valid_kernel_args()
        dy = torch.randn(a["query"].shape[0], 32, device="cuda")
        with pytest.raises(RuntimeError):
            _C.paged_attention_committed(dy, a["k_storage"], a["v_storage"],
                                          a["scales_k"], a["scales_v"], a["block_table"], a["inv_sqrt_d"])

    def test_k_v_page_count_mismatch_rejected(eb):
        a = eb._valid_kernel_args()
        ea = a["v_storage"][:1]
        with pytest.raises(RuntimeError):
            _C.paged_attention_committed(a["query"], a["k_storage"], ea,
                                          a["scales_k"], a["scales_v"], a["block_table"], a["inv_sqrt_d"])

    def test_k_v_head_count_mismatch_rejected(ed):
        a = ed._valid_kernel_args(num_heads=4)
    ...
    def test_k_v_page_size_mismatch_rejected(ef):
        a = ef._valid_kernel_args(page_size=16)
    ...
    def test_page_size_exceeding_kernel_max_rejected(ep):

        ek, ei, el = 4, 64, 128
        eh = PagedKVCache(num_layers=1, ek=ek, ei=ei,
                              el=el, max_pages=1, device="cuda")
    ...
    def test_runtime_page_size_4_and_16_both_supported(et):


        for es in (4, 16):
            a = et._valid_kernel_args(es=es, max_pages=2)
            out = _C.paged_attention_committed(a["query"], a["k_storage"], a["v_storage"],
                                                a["scales_k"], a["scales_v"], a["block_table"], a["inv_sqrt_d"])
            ev, eu, er = out
...
class TestEmptyAttentionCache:
    def test_paged_attention_on_never_appended_cache_raises_value_error(ey):
        ...
        with pytest.raises(ValueError):
            paged_attention(ew, layer_idx=0, ex=ex)


class TestInvalidScale:


    def _committed_cache(fa):
    ...
    def test_paged_attention_rejects_invalid_scale(fe, fb):
        fc = fe._committed_cache()
        fd = torch.randn(4, 64, device="cuda")
        with pytest.raises(ValueError):
            paged_attention(fc, layer_idx=0, fd=fd, scale=fb)

    @pytest.mark.parametrize("good_scale", [1.0, 0.125, 1e-6, 1e6])
    def test_paged_attention_accepts_valid_scale(fi, fg):
        ff = fi._committed_cache()
        fh = torch.randn(4, 64, device="cuda")
        out = paged_attention(ff, layer_idx=0, fh=fh, scale=fg)
        assert torch.isfinite(out).all()

    @pytest.mark.parametrize("bad_scale", [0.0, -1.0, float("nan"), float("inf"), float("-inf")])
    def test_raw_kernel_rejects_invalid_scale(fq, fj):
        fl = fq._committed_cache()
...
class TestCacheConstructorValidation:
    ...
    def test_layer_cache_rejects_bad_num_heads(ft, fs):
        with pytest.raises(ValueError):
            LayerKVCache(fs=fs, head_dim=8, page_size=16, max_pages=2, device="cuda")

    @pytest.mark.parametrize("head_dim", [0, -1])
    def test_layer_cache_rejects_bad_head_dim(fv, fu):
        with pytest.raises(ValueError):
            LayerKVCache(num_heads=4, fu=fu, page_size=16, max_pages=2, device="cuda")

    @pytest.mark.parametrize("page_size", [0, -1])
    def test_layer_cache_rejects_bad_page_size(fx, fw):
        with pytest.raises(ValueError):
            LayerKVCache(num_heads=4, head_dim=8, fw=fw, max_pages=2, device="cuda")

    @pytest.mark.parametrize("max_pages", [0, -1])
    def test_layer_cache_rejects_bad_max_pages(fz, fy):
        with pytest.raises(ValueError):
            LayerKVCache(num_heads=4, head_dim=8, page_size=16, fy=fy, device="cuda")

    @pytest.mark.parametrize("num_layers", [0, -1])
    def test_paged_cache_rejects_bad_num_layers(gb, ga):
        with pytest.raises(ValueError):
            PagedKVCache(ga=ga, num_heads=4, head_dim=8, page_size=16, max_pages=2, device="cuda")
...
class TestKVMismatch:
    ...
    def test_token_count_mismatch_rejected(ge):
        gd = ge._cache()
    ...
    def test_trailing_shape_mismatch_rejected(gg):
        gf = gg._cache()
    ...
    def test_wrong_rank_rejected(gi):
        gh = gi._cache()
    ...
    def test_wrong_head_count_rejected(gk):
        gj = gk._cache()
    ...
    def test_wrong_head_dim_rejected(gm):
        gl = gm._cache()
    ...
    def test_layer_idx_out_of_range_rejected(go):
        gn = go._cache()
...
class TestPageBoundaryTransitions:
    ...
    def test_seq_len_boundary(gv, gt, gw):
        if gw == 0:
            pytest.skip("0-token append is covered by TestZeroTokenOperations")
        gs = gw // gt + 2
        gp = LayerKVCache(num_heads=2, head_dim=8, gt=gt, gs=gs, device="cuda")
        gp.append(_random_tokens(gw, num_heads=2, head_dim=8, seed=gw))

        gq = gw // gt
        gr = gw % gt

        assert gp.seq_len == gw
        assert len(gp.block_table) == gq
        assert len(gp._staging) == gr

        gu = gp.read_fp32()
        assert gu.shape[0] == gw


class TestAttentionAtRuntimePageSize4:


    def test_committed_only(hi):
        hb, gy, hc = 4, 64, 4
        ha = 2 * hc
        gx = PagedKVCache(num_layers=1, hb=hb, gy=gy,
                              hc=hc, max_pages=4, device="cuda")
        ...
        got = paged_attention(gx, layer_idx=0, he=he)
    ...
    def test_staging_only(hs):
        ho, hl, hp = 4, 64, 4
        hn = hp - 1
        hk = PagedKVCache(num_layers=1, ho=ho, hl=hl,
                              hp=hp, max_pages=4, device="cuda")
        ...
        got = paged_attention(hk, layer_idx=0, hq=hq)
        hr = _reference_full_attention(hq, hm, hu, 1.0 / (hl ** 0.5))
        torch.testing.assert_close(got, hr, atol=1e-4, rtol=1e-4)

    def test_mixed_committed_and_staging(ij):
        ic, hy, id = 4, 64, 4
        ia, ib = id, 3
        hv = PagedKVCache(num_layers=1, ic=ic, hy=hy,
                              id=id, max_pages=4, device="cuda")
        ...
        got = paged_attention(hv, layer_idx=0, ie=ie)
...
class TestGQARatios:
    ...
    def test_valid_ratio_succeeds(iw, it, ir):
        ip, iu = 64, 16
        io = PagedKVCache(num_layers=1, num_heads=ir, ip=ip,
                              iu=iu, max_pages=2, device="cuda")
        iq = _random_tokens(iu, ir, ip, seed=it + ir)
        ix = _random_tokens(iu, ir, ip, seed=it - ir)
        io.append(0, iq, ix)
        iv = torch.randn(it, ip, device="cuda")
        out = paged_attention(io, layer_idx=0, iv=iv)
        assert out.shape == (it, ip)
        assert torch.isfinite(out).all()

    def test_invalid_ratio_rejected_by_wrapper(jd):

        iz, jb, ja = 64, 16, 4
        iy = PagedKVCache(num_layers=1, num_heads=ja, iz=iz,
                              jb=jb, max_pages=2, device="cuda")
        iy.append(0, _random_tokens(jb, ja, iz, seed=30),
                     _random_tokens(jb, ja, iz, seed=31))
        jc = torch.randn(7, iz, device="cuda")
        with pytest.raises(ValueError):
            paged_attention(iy, layer_idx=0, jc=jc)

    def test_invalid_ratio_rejected_by_raw_kernel(jn):
        jg, jj, ji = 64, 16, 4
        jf = PagedKVCache(num_layers=1, num_heads=ji, jg=jg,
                              jj=jj, max_pages=2, device="cuda")
...
class TestNonContiguousInputs:
    ...
    def test_quantize_input_supports_non_contiguous(jv):
    ...
    def test_quantize_page_storage_rejects_non_contiguous(jy):
    ...
    def test_paged_attention_query_supports_non_contiguous(kg):
        ke, kd, kf = 4, 64, 16
        kc = PagedKVCache(num_layers=1, ke=ke, kd=kd,
                              kf=kf, max_pages=2, device="cuda")
    ...
    def test_paged_attention_k_storage_rejects_non_contiguous(ks):
        km, kk, ko = 4, 64, 16
        kj = PagedKVCache(num_layers=1, km=km, kk=kk,
                              ko=ko, max_pages=2, device="cuda")
...
class TestNumericalEdgeCases:
    ...
    def test_values_beyond_int8_range_clamp_to_127(lj):
    ...
    def test_negative_values_beyond_int8_range_clamp_to_neg_127(lm):
    ...
    def test_extreme_finite_value_with_tiny_scale_does_not_crash(lq):
    ...
    def test_nan_input_does_not_crash_and_stays_in_range(lu):
    ...
    def test_inf_input_does_not_crash_and_stays_in_range(lz, lw):
        ma = torch.full((1, 2, 4), lw, device="cuda")
    ...
    def test_new_scale_with_inf_in_tensor_does_not_crash(me):
